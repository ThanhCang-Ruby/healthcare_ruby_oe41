class Admin::AccountsController < ApplicationController
  ...
  def create
    ...
    if @account.save && @account.update_columns(status: 2, activated_at: Time.zone.now, role: 2)
  ...
  def blcok
    @customer.update_column :status, 0
  ...
  def load_staffs
    @staffs = Account.staff.includes(license: :major)
                     .sort_by_create_at.paginate page: params[:page],
                        per_page: Settings.account.staff.per_page
  end

  def load_customers
    @customers = Account.customer.sort_by_create_at
